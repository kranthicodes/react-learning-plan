...
export default class Parent extends Component {
  state = {
    ...
    colors: [],
    selectedColor: "#8de2e7"
  };

  componentDidMount() {
    let colors = this.state.users.map(user => {
  ...
  changeHandler = e => {
    if (e.target.type === "text") {
      ...
      let selectedColor = this.state.colors[
  ...
  render() {
    const { todos, users, selectedUser, textField, selectedColor } = this.state;
    return (
      <Fragment>
        <div
